...
Most of the variables of the dataset are continuous, but it is possible to apply the Naïve Bayes predictor after a discretization process. To do so, certain ranges of the data will be transformed into categories. This must be done for all variables, but each variable will require specific adjustments depending on its range of values. For example, variable `Number_seasons` ranges from 10 to 26. The histogram in figure ?? shows how this variable is distributed.

```{r echo=F}
baseball.dataset.binary <- data.frame(baseball.dataset)
levels(baseball.dataset.binary$Hall_of_Fame) <- c("no", "yes", "yes")
barplot(table(baseball.dataset.binary$Hall_of_Fame, baseball.dataset.binary$Number_seasons), col=rev(brewer.pal(4, "Purples"))[1:2])
legend("topright", legend=c("Hall of Fame", "Not Hall of Fame"), fill=rev(brewer.pal(4, "Purples"))[1:2])
```
...
```{r echo=F, fig.cap="\\label{fig:roc-table-log-regression}ROC table of the best logistic regression model"}
